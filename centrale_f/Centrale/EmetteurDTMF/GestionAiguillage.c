...
extern void Generator_Cmd_Vitesse(uint8_t robot_id, uint8_t speed);
...
void gestion_intersection_process(void) {
    ...
    if (robot_waiting_on_entrance != -1) {
        if (intersection_busy == 0) {
            ...
            robots_db[waiting_id].at_intersection_entrance = 0;
            ...
            Generator_Cmd_Vitesse(waiting_id, 40);
        }
        return;
    }

    for (uint8_t id = 0; id < MAX_ROBOTS; id++) {
        if (robots_db[id].robot_id == id && robots_db[id].at_intersection_entrance > 0) {
